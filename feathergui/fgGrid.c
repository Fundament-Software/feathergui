...
FG_EXTERN void FG_FASTCALL fgGrid_Reposition(fgGrid* self, fgChild* p)
{
  ...
  if((self->window.flags&FGGRID_TILEX)!=0 == (self->window.flags&FGGRID_TILEY)!=0) { // Either TILEX and TILEY are both on or both off
    if(self->window.flags&FGWIN_EXPANDX) { // If we're expanding on the X axis we must grow downward and then loop around.
      dim=self->window.element.element.area.bottom.abs-self->window.element.element.area.top.abs-self->margins.bottom;
      mov.x=self->margins.left;
      while(p)
      {
        mov.y+=self->margins.top;
        if(mov.y>dim) mov.x=self->margins.left+self->margins.right+rt->right.abs;
      ...
      dim=self->window.element.element.area.right.abs-self->window.element.element.area.left.abs-self->margins.right;
      mov.y=self->margins.top;
      while(p)
      {
        mov.x+=self->margins.left;
        if(mov.x>dim) mov.y=self->margins.top+self->margins.bottom+rt->bottom.abs;
        rt=&p->element.area; // set rt down here so we use the previous value above
        dx=rt->right.abs-rt->left.abs;
        dy=rt->bottom.abs-rt->top.abs;
        rt->left.abs=mov.x;
  ...
  } else if((self->window.flags&FGGRID_TILEX)!=0) { // Only TILEX is active
    while(p)
    {
      rt=&p->element.area;
      mov.x+=self->margins.left;
  ...
  if(self->window.flags&FGWIN_EXPANDX) narea.right.abs=narea.left.abs+mov.x;
  if(self->window.flags&FGWIN_EXPANDY) narea.bottom.abs=narea.top.abs+mov.y;
