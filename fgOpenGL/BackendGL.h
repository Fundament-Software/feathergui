...
namespace GL {
  ...
  public:
    ...
    static int ClearDepthStencil(FG_Backend* self, void* commands, FG_Resource* depthstencil, char clear, uint8_t stencil,
    ...
    static FG_Resource* CreateBuffer(FG_Backend* self, FG_Context* context, void* data, uint32_t bytes,
                                     enum FG_Type type);
    static FG_Resource* CreateTexture(FG_Backend* self, FG_Context* context, FG_Vec2i size, enum FG_Type type, enum FG_PixelFormat format,
    ...
    static int PutClipboard(FG_Backend* self, FG_Window* window, enum FG_Clipboard kind, const char* data,
                            uint32_t count);
    static uint32_t GetClipboard(FG_Backend* self, FG_Window* window, enum FG_Clipboard kind, void* target,
                                 uint32_t count);
