...
Asset::Asset(fgFlag flags, cli::array<System::Byte>^ data, size_t length) : _owner(true), _p(0)
{
  ...
  _p = fgSingleton()->backend.fgCreateAsset(flags, (char*)pstr, data->Length);
}
...
PointF^ Asset::Size(UnifiedRect^ uv, fgFlag flags)
{
  AbsVec out = { 0,0 };
  fgSingleton()->backend.fgAssetSize(_p, &uv->operator CRect(), &out, flags);
