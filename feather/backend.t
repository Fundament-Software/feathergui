...
terra B.Backend:DrawCircle(window : &opaque, area : &F.Rect, fillColor : F.Color, border : float, borderColor : F.Color, blur : float, innerRadius : float, innerBorder : float, asset : &B.Asset, z : float, blendstate : &B.BlendState) : F.Err return 0 end
terra B.Backend:DrawArc(window : &opaque, area : &F.Rect, angles : F.Vec, fillColor : F.Color, border : float, borderColor : F.Color, blur : float, innerRadius : float, asset : &B.Asset, z : float, blendstate : &B.BlendState) : F.Err return 0 end
